//This is the new version of OpenCLBackProjection.cl
...
kernel void OpenCLBackProjection(
	__global float* backProjectionPic,			//OpenCL reconstructed phantom
	...
	float originY)								//Origin for the detector/CT coordinate system in y direction
{
	...
	for (int a = 0; a < numberProj; a++)		//Loop over the number of projections
	{
		...
		//Validating the angle to simplify computation and to avoid indetermined results in trigonometric functions. (Tan)
        ...
        else
        {
			...
			//Finding the interception coordinates between the ray passing through the considered phantom pixel and the detector.
		...
		detIndexMF = (s + (detectorLength/2))/detectorSpacing;	//Transforming the distance from the central ray to a computer space index. (index for the sinogram line)
		double valSin = 0;
