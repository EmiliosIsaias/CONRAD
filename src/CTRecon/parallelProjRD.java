...
public class parallelProjRD {
    ...
    public parallelProjRD(int projectionNumber, double detectorSpacing, int detectorPixel)
    {
    ...
    public Grid2D projectRayDriven(Grid2D grid) 
    {
        ...
        double detSize = this.detectorSpacing * this.detectorPixel;    //Detector size in millimeters
        ...
        for(int a = 0;a<= projectionNumber;++a)
        {
            ...
            for(int i = 0;i<=detectorPixel;++i)
            {
                ...
                PointND P1 = new PointND(s*cT - sT,s*sT + cT,.0d);	//Point sitting on the orthogonal line with respect to the detector.
