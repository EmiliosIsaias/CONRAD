...
public class parallelBP_PD 
{
	
	int angleIndex;
	double Dtheta, maxTheta;
	
	int sIndex, detectorPixel;
	...
	public parallelBP_PD (int projectionNumber, double detectorSpacing, int detectorPixel)
	{
	...
	public void setSinogramParameters(Grid2D sino) 
	{
		...
		this.detectorLength = sIndex*detectorSpacing;
	}
	/*!
		...
		for(int i = 0; i <= grid.getSize()[0]; ++i)
		{
			...
			for(int j = 0; j <= grid.getSize()[1]; ++j)
			{
				...
				for(int a = 0;a< projectionNumber;++a)
				{
					...
					double yIntercept;
					if(projAngle == 0 || projAngle == (Math.PI/2) || projAngle == Math.PI)
					{
						if(projAngle == (Math.PI/2)){s = y;}
						else{s = x;}
					}
					else{
						...
						if(magn_dPX == 0 || detDir == 0){
							...
							cosAlpha = (dtX*x_det + dtY*y_det)/(magn_dPX*detDir);	//Angle between these two vectors. (0° or 180°)
						}
						s = magn_dPX * Math.signum(cosAlpha);
					}					
					detIndex = (s + (detectorLength/2))/detectorSpacing;	//Detector index computing.					
					...
					sum += pixelFromSino;										//Interpolated value from the sinogram and weighted for redundancy.
				}
	...
	public Grid2D filterSino(Grid2D sino)
    {
    	...
    	{
    		if(1==(Math.abs(p)%2)){
    			rampFilter.setAtIndex(p+(filterSize/2), (float) (aux/(Math.pow(p, 2))));}
    		if (p==0){
    			rampFilter.setAtIndex(p+(filterSize/2), (float) (1.0/4.0));}
    	}
    	rampFilter.transformForward();
    	VisualizationUtil.createPlot(rampFilter.getSubGrid(0, filterSize).getBuffer()).show();
